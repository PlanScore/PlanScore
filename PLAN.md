...
## Overview
Currently, `model_votes()` assigns individual incumbency scenarios to each district and returns `(sims, districts, 2)`. After changes, it will generate votes for ALL three incumbency scenarios and return `(3, sims, districts, 2)`, with `calculate_district_biases()` selecting the appropriate scenario per district.
...
- Stack results into shape `(3, sims, districts, 2)` where dimension 0 indexes incumbency scenario
- Update docstring to reflect new return shape

**Key line references:**
- Line 171-175: Docstring "Return is a SxDx2 matrix" → Change to "Return is a 3xSxDx2 matrix where first dimension is incumbency scenario (Republican=-1, Open=0, Democrat=1)"
...
incumbency_scenarios = [-1, 0, 1]  # Republican, Open, Democrat (matches INCUMBENCY_INDEX order)
all_fractions = []
...
# Build 3xSxDx2 array with per-party vote totals for each incumbency, simulation, district, and party
votes_dem = (all_fractions * scale).round(1)  # (3, sims, districts)
votes_rep = ((1 - all_fractions) * scale).round(1)  # (3, sims, districts)
votes = numpy.stack([votes_dem, votes_rep], axis=3)  # (3, sims, districts, 2)
...
**Required changes:**
- After line 982, receive `(3, sims, districts, 2)` from `model_votes()`
- Add incumbency selection logic using `upload.incumbents` and `INCUMBENCY_INDEX` constant
...
# output_votes shape is now (3, sims, districts, 2)
...
# Select the correct incumbency scenario for each district using INCUMBENCY_INDEX
selected_votes = numpy.zeros((sim_count, district_count, 2))
for i, incumbency in enumerate(upload.incumbents):
    idx = INCUMBENCY_INDEX[incumbency]
...
incumbency_indices = numpy.array([INCUMBENCY_INDEX[inc] for inc in upload.incumbents])
...
**Current changes:**
- Line 348-353: Update shape expectations to `(3, sims, districts, 2)`
...
**Current changes:**
- Line 1355-1359: Mock return shape changes to `(3, sims, districts, 2)`:
```python
...
**Current changes:**
- Line 1469-1473: Update mock return shape to `(3, sims, districts, 2)`

#### 9. **planscore/tests/test_score.py::test_calculate_gap_unified_incumbents()** (lines 1559-1590)
**Current changes:**
- Line 1580-1584: Update mock return shape to `(3, sims, districts, 2)` with **different values per scenario**
- Lines 1587-1590: **Remove** these assertions - they checked incumbency passed to `model_votes`
- **Add** new assertions to verify correct incumbency scenario selection using `INCUMBENCY_INDEX`
...
**Current changes:**
- Line 1641-1645: Update mock return shape to `(3, sims, districts, 2)`

#### 11. **planscore/tests/test_score.py::test_calculate_gap_with_zeros()** (lines 1668-1762)
**Current changes:**
- Line 1689-1693: Update mock return shape to `(3, sims, districts, 2)`
- Ensure NaN districts work across all incumbency scenarios

## Implementation Order

**Recommended sequence:**
1. Add `INCUMBENCY_INDEX` constant to score.py (after line 26)
2. Modify `model_votes()` to generate all 3 incumbency scenarios in order [-1, 0, 1]
3. Add incumbency selection logic to `calculate_district_biases()` using `INCUMBENCY_INDEX`
4. Update all test mocks to use `(3, sims, districts, 2)` shape
...
- **Core:** 1 new constant (INCUMBENCY_INDEX), 2 functions (model_votes, calculate_district_biases)
- **Tests:** 8 test functions

**Breaking changes:** Yes - `model_votes()` return shape changes from `(sims, districts, 2)` to `(3, sims, districts, 2)`

**Key design decisions:**
- Array stacking order matches model incumbency values: [-1, 0, 1] → Republican, Open, Democrat
- `INCUMBENCY_INDEX` constant placed in score.py where it's used, mirrors matrix.INCUMBENCY pattern
