...
live-lambda: planscore-lambda.zip
	parallel \
		...
		:::  $(LAMBDA_PREFIX)-UploadFields $(LAMBDA_PREFIX)-AfterUpload $(LAMBDA_PREFIX)-RunDistrict $(LAMBDA_PREFIX)-ScoreDistrictPlan \
		>> /dev/null
	aws --region us-east-1 lambda update-function-configuration --dead-letter-config TargetArn=$(AWS_LAMBDA_DLQ_ARN) \
	    --function-name $(LAMBDA_PREFIX)-UploadFields --handler lambda.upload_fields --timeout 3 >> /dev/null
	aws --region us-east-1 lambda update-function-configuration --dead-letter-config TargetArn=$(AWS_LAMBDA_DLQ_ARN) \
	    --function-name $(LAMBDA_PREFIX)-AfterUpload --handler lambda.after_upload --timeout 30 >> /dev/null
	aws --region us-east-1 lambda update-function-configuration --dead-letter-config TargetArn=$(AWS_LAMBDA_DLQ_ARN) \
	    --function-name $(LAMBDA_PREFIX)-RunDistrict --handler lambda.run_district --timeout 300 >> /dev/null
	aws --region us-east-1 lambda update-function-configuration --dead-letter-config TargetArn=$(AWS_LAMBDA_DLQ_ARN) \
	    --function-name $(LAMBDA_PREFIX)-ScoreDistrictPlan --handler lambda.score_plan --timeout 30 >> /dev/null
