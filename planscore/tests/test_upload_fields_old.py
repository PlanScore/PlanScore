...
from .. import upload_fields_old, constants
...
class TestUploadFields (unittest.TestCase):
    ...
    def test_generate_signed_id(self):
        unsigned_id, signed_id = upload_fields_old.generate_signed_id('secret')
    ...
    @unittest.mock.patch('planscore.upload_fields_old.get_assumed_role')
    @unittest.mock.patch('planscore.upload_fields_old.get_upload_fields')
    def test_lambda_handler(self, get_upload_fields, get_assumed_role, event_url):
        ...
        response = upload_fields_old.lambda_handler({}, None)
    ...
    @unittest.mock.patch('planscore.upload_fields_old.generate_signed_id')
    def test_with_token(self, generate_signed_id):
        ...
        url, fields = upload_fields_old.get_upload_fields(s3, creds, 'sec')
        ...
        s3.generate_presigned_post.assert_called_once_with('the-bucket',
            ...
            {'success_action_redirect': 'https://example.org/annotate-old.html?id=id.sig'},
            ['starts-with', '$key', 'uploads/id/upload/']], ExpiresIn=300)

        generate_signed_id.assert_called_once_with('sec')
        self.assertEqual(fields['success_action_redirect'], 'https://example.org/annotate-old.html?id=id.sig')
